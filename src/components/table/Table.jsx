    ...
    return (
        <>
            <div className=' sm:px-0 sm:flex justify-center' >
                {item.reason === '1' ?
                    <div onClick={handleClick} className={`cursor-pointer px-1  bg-white mt-1 w-full sm:w-[600px] shadow shadow-black border-r-8 border-r-green-600  ${!clicked ? "grid grid-cols-12" : "opcty3"}`}>
                    ...
                    <div onClick={handleClick} className={`cursor-pointer px-1   bg-white mt-1 w-full sm:w-[600px] shadow shadow-black border-r-8 border-r-red-600 ${!clicked ? "grid grid-cols-12" : ""}`}>
                        <div className='col-span-5 flex justify-between'>
                            <span>{item?.subject}</span>
                            <span onClick={() => handleRemove(item?.id)} className={`text-white ${!clicked ? "hidden" : "block"} bg-red-500 m-1 p-1 rounded-sm`}>remove</span>
                        </div>
